...
simul$options <- list("tp",
  anLeg = 2016, pas1 = 3 / 12, pas2 = 1,
...
simul$mortadiff_dip_F <- xlsx::read.xlsx(paste0(".\\inst\\extdata\\Param_demo_2017\\MORTA_DIP.xls"), sheetName = "morta_dif_F", startRow = 1)
simul$mortadiff_dip_H <- xlsx::read.xlsx(paste0(".\\inst\\extdata\\Param_demo_2017\\MORTA_DIP.xls"), sheetName = "morta_dif_H", startRow = 1)
NA0 <- function(x) {
  return(ifelse(is.na(x), 0, x))
}
simul$CiblesDemo <- xlsx::read.xlsx(paste0(".\\inst\\extdata\\Param_demo_2017\\ciblesDemographie_", champ, ".xls"), sheetName = "CiblesDemo", startRow = 2, colClasses = rep("numeric", 48)) %>%
  mutate_all(funs(NA0))
simul$FinEtudeMoy <- xlsx::read.xlsx(".\\inst\\extdata\\PARAM_etude.xls", sheetName = "TABLEFINDET0", startRow = 2)
simul$mortalite_diff <- xlsx::read.xlsx(".\\inst\\extdata\\PARAM_Mortalite_diff.xls", sheetName = "Mortalite_diff", startRow = 2)
simul$EqSalaires <- xlsx::read.xlsx(".\\inst\\extdata\\EqSalaires.xls", sheetName = "eq_salaires")
simul$CStructSexeAge <- xlsx::read.xlsx(".\\inst\\extdata\\EqSalaires.xls", sheetName = "CorrectStructSalSexeAge")
simul$EqTrans <- xlsx::read.xlsx(".\\inst\\extdata\\EqTrans.xls", "EqTrans") %>%
  mutate(indic = as.integer(indic)) %>%
  mutate(ordre = as.factor(paste0("TRANS", ordre + 1))) %>%
  arrange(type_trans, origine, ordre)
simul$EqSante <- xlsx::read.xlsx(".\\inst\\extdata\\sante.xls", sheetName = "adl")
###################
# chargement des paramètres eco
##################
scencho <- "7%" # ou 4,5 ou 10
scenprod <- "1,3%" # ou 1,0 ou 1,5 ou 1,8
ciblesTrans <- ".\\inst\\extdata\\COR_2018\\PARAM_transitions.xls"
param_eco <- ".\\inst\\extdata\\COR_2018\\ParamSociaux_2018.xls"
simul$macro <-
...
ech <- ech[, c(
  "sexe", "Id", "findet", "typeFP", "anaiss", "neFrance",
