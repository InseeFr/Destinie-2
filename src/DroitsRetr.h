...
public:
  ...
  double tauxliq_fp = 0,     ///< taux de liquidation à la FP (=1 en cas de départ sans décote ni surcote)       
    tauxliq_rg = 0,          ///< taux de liquidation au RG (=1 en cas de départ sans décote ni surcote) et au régime des indépendants 
    tauxliq_ar = 0;          ///< taux de liquidation à l'ARRCO et à l'AGIRC (=1 en cas de départ sans décote ni surcote) 
   ...
   *   c'est-à-dire au trimestre près) 
   * 
   *  Cette fonction nécessite lance auparavant les fonctions durees_majo et durees_base
   * 
   *  Cette méthode renseigne les variables suivantes :
   *  - \ref dursurcote , \ref durdecote_fp , \ref dursurcote_fp , \ref durdecote_rg
   *  - \ref tauxliq_rg , \ref tauxliq_fp , \ref tauxliq_ar
   ...
   *  - du taux de liquidation (coefficient d'abattement) dans les régimes complémentaires ARRCO et AGIRC (suppression de la décote si taux plein au RG)
   */
  void durees_et_decote_surcote();
  
  /** \brief Retourne l'âge minimal permettant un départ à la retraite.
   *    @return Un double correspondant à l'âge minimal de départ à la retraite possible.
   * 
   *  Cet âge est vraisemblablement différent de l'âge d'ouverture des droits car
   *  un individu peut également partir au titre de l'invalidité ou selon le
   *  dispositif de retraite anticipée carrière longue.
   *  Aucun calcul d'éligibilité n'est effectué ou de départ effectif n'est réalisé,
   *  cette fonction est surtout une optimisation pour ne pas faire de test dans
   *  des situations où il est impossible de liquider ses droits.
   * 
  ...
  /** 
   *  \brief Retourne l'âge minimal d'ouverture des droits. 
   *  @return Un double correspondant à l'âge minimal d'ouverture des droits.
   * 
   *  Remarque: fait appel aux méthodes \ref durees_base, \ref durees_majo et \ref DecoteSurcote
   */
  double AgeMin(); 
  ...
  double duree_emp_reg(int regime) {
    ...
    if(regime==REG_IN) return duree_in;
    if(regime==REG_FP) return duree_fp;
    if(regime==REG_AR) return (
      (l.an_leg < 2015 || t < 119 || options->NoRegUniqAgircArrco) ?
    duree_emp - duree_fpa - duree_fps - duree_in :
      duree_trim(Statuts_occ, 2018) - duree_trim(statuts_fpa, 2018) - duree_trim(statuts_fps, 2018)
      - (options->NoAssimil ? duree_trim(statuts_ind,2018) : duree_trim(statuts_ind_assimil,2018))
    );
    if(regime==REG_AG) return duree_ag;
    if(regime==REG_AGIRC_ARRCO) return (
      (l.an_leg < 2015 || t < 119 || options->NoRegUniqAgircArrco) ?
    0 :
      duree_emp - duree_fpa - duree_fps - duree_in 
    - (duree_trim(Statuts_occ, 2018) - duree_trim(statuts_fpa, 2018) - duree_trim(statuts_fps, 2018) 
