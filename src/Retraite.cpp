...
void Retraite::revaloDir(int t)
{
  ...
  if(totliq && age >= totliq->ageliq) {
	  ...
	  if((!options->NoCoeffTemp) && (!options->NoAccordAgircArrco) &&  (t >=119) && (X.anaiss >=1957)) {
...
void Retraite::revaloDer(int t)
{
  ...
  rev_ag_ar = liqrev->rev_ag_ar   * M->ValPtAGIRC_ARRCO[t]  / M->ValPtAGIRC_ARRCO[t_liq];
  ...
  if (age<60 && !in(X.statuts[age],{S_INVAL, S_INVALRG, S_INVALFP, S_INVALIND}) && liqrev->nbEnfCharge <=1){
	  rev_ag=0.0;
  }
  ...
  else {excedent=X.retr->revenuReversion() + rev_in/(1+tauxmajo_in) + rev_rg/(1+tauxmajo_rg) - M->PlafRevRG[t];}  // on vérifie la condition de ressources sans l'ajout éventuel de la majoration pour enfants dans Reversion.cpp
  ...
  if ((totliq && age==totliq->ageliq)||(totliq && age==liqrev->ageliq_rev )){
    // Amputation des reversions RG et ind. si ressources dépassent
    // plafond 
    double excedent = 0;
    double ratio_rg = 0;
    if ((rev_rg + rev_in) > 0)
    {
      if (matri==MARIE) {excedent=X.retr->revenuReversionCouple() + rev_in/(1+tauxmajo_in) +rev_rg/(1+tauxmajo_rg) - 1.6*M->PlafRevRG[t];}
      else {excedent=X.retr->revenuReversion() + rev_in/(1+tauxmajo_in) + rev_rg/(1+tauxmajo_rg) - M->PlafRevRG[t];} 
      ratio_rg = rev_rg / (rev_rg+rev_in);
      if (excedent>0){ 
      rev_rg = (1+tauxmajo_rg)*max(0.0, rev_rg/(1+tauxmajo_rg) - excedent * ratio_rg);
      rev_in = (1+tauxmajo_in)*max(0.0, rev_in/(1+tauxmajo_in) - excedent * (1-ratio_rg));
  ...
  /*if(totliq && (age>=l.AgeAnnDecRG) && matri!=MARIE) { // il reste à rajouter la condition sur le plafond
	rev_rg*=(1+11.1/100.0);
	rev_in*=(1+11.1/100.0);
  } */
...
if(totliq && age >= totliq->ageliq) {
	...
	if((!options->NoCoeffTemp) && (!options->NoAccordAgircArrco) &&  (annee >=119) && (X.anaiss >=1957)) {
