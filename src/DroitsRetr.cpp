...
{
  // RMQ sur les âges :
  datetest = arr_mois(X.anaiss + agetest, X.moisnaiss);
  age     = int_mois(agetest, X.moisnaiss); // âge atteint l'année de l'agetest
  t = X.date(age);
}
...
void DroitsRetr::durees_base() {  
  ...
  duree_ag     =  (l.an_leg < 2015 || t < 119 || options->NoRegUniqAgircArrco)? duree_trim(statuts_cad) : duree_trim(statuts_cad,2018);
  ...
  if (l.an_leg < 2012 || datetest <  arr_mois(2012, 10)) {
    ...
    dureecotdra_tot = duree_emp + min(duree_cho, 0.5) + min(duree_snat,1.0);
  }
  else if (l.an_leg < 2023 || t < 123) {
    dureecotdra_tot = duree_emp + min(duree_cho, 1.0) + min(duree_inv, 0.5) + min(duree_snat, 1.0);
  }
  else {
    dureecotdra_tot = duree_emp + min(duree_cho, 1.0) + min(duree_inv, 0.5) + min(duree_snat, 1.0) + min(duree_avpf, 1.0);
  }
  ...
  if(l.an_leg < 2015 || t < 119 || options->NoRegUniqAgircArrco) { // A revoir: faut-il prendre en compte les points acquis en cas de chômage indemnisé ?
...
{
  int agedebsurcote = int(l.AgeSurcote);
  ...
  for(int a=a_min; a <= age; a++) {
    ...
    else if (a == agedebsurcote && a==age) dursurcote += max(0.0, ArrTrimInf(agetest - l.AgeSurcote));
    else if (a == agedebsurcote && a <age) dursurcote += max(0.0, agedebsurcote + 1 - ArrTrimSup(l.AgeSurcote));
    else if (a<age                       ) dursurcote += 1 ;
    else if (a==age                      ) dursurcote += ArrTrimInf(agetest - age);
  }
  if (l.surcote_parentale && agedebsurcote - 1 <= age && in(X.statuts[agedebsurcote - 1], Statuts_occ)) {
    dursurcote += 1;
  }
  ...
  if ((in(X.statuts[agedebsurcote - 1], Statuts_occ) || in(X.statuts[agedebsurcote], Statuts_occ)) && (t>103) && (l.an_leg>2003))
  ...
  if (!X.retr->primoliq)
  {
    if (arr_mois(agetest) < arr_mois(AgeMinGlobal()))   
      tauxliq_fp = 0; 
    else  
  ...
  tauxliq_rg = (l.TauxPleinRG-l.DecoteRG * durdecote_rg
                  ...
                  if (agetest >= AgeMinGlobal() && tauxliq_rg >= 1)   
                    tauxliq_ar = 1;
}


double DroitsRetr::AgeMinGlobal() {
  double age_min = min(l.AgeMinRG, l.AgeMinFP);
  age_min = min(age_min, l.AgeMinInv);
  // Présuppose que l'AgeDRA minimal correspond à la première entrée
  age_min = min(age_min, l.AgeDRA[0]);
  
  return age_min;
}

void DroitsRetr::durees_et_decote_surcote() {
  durees_base();
  durees_majo();
  DecoteSurcote();
  return;
...
double DroitsRetr::AgeMin()
{
  return min(l.AgeMinRG, l.AgeMinFP);
}
...
void DroitsRetr::SalBase(int AnneeRefAnticip) {
  ...
  // Ici le SAM est quand même calculé si potentiellement en âge de liquider au FP
  if (arr_mois(agetest) < arr_mois(AgeMinGlobal())) return;
...
void DroitsRetr::Points(int AnneeRefAnticip) { // AnneeRefAnticip == Année de référence anticipée. Utilisée dans TestLiq afin de prendre en compte l'anticipation de l'évolution du salaire de référence.
  ...
  for(int a = age_deb; a <= age_max; a++) {
    ...
    if(u < 119 || l.an_leg < 2015 || options->NoRegUniqAgircArrco) {
...
void DroitsRetr::AppliqueBonif()
{
  ...
  if (!X.retr->totliq && (pension_rg + pension_ar + pension_ag + pension_in > 0) )  
  {
    ...
    if ((l.an_leg>=2015) && (t>=119) && !options->NoRegUniqAgircArrco){
...
void DroitsRetr::MinCont(int AnneeRefAnticip)
{
  ...
  double dureecotmin_tot_avpf = dureecotmin_tot + min(duree_avpf, 6.0);
  ...
  double taux_prorat_cotmin_avpf = dureecotmin_tot_avpf/l.DureeProratRG; 
  ...
  if( t > 83 && min(t, l.LegMin%1900) < 104) {
    ...
    else if(  t<123 || l.an_leg < 2023) { // ajout le 06-09-2013 pour prendre en compte réforme de 2013
    ...
    else{// Ajout pour la réforme de 2023 : prise en compte AVPF pour éligibilité et calcul majoration mico
      if(dureecotmin_tot_avpf < 120/4.0)
        majo = 0;
      min_cont    = (mico1 + majo * min(taux_prorat_deplaf, taux_prorat_cotmin_avpf))
        * duree_rg_maj / (duree_rg_maj + duree_in_maj)
        * min(1.0, taux_tot_rg);
        min_cont_in = (mico1 + majo * min(taux_prorat_deplaf, taux_prorat_cotmin_avpf))
          * duree_in_maj / (duree_rg_maj + duree_in_maj)
          * min(1.0, taux_tot_in);
...
void DroitsRetr::LiqPrive(int AnneeRefAnticip)
{
  ...
  double taux = tauxliq_rg * l.TauxPleinRG;
  // mise à zero du taux si avant aod dans le cas général.
  if (
      agetest < l.AgeMinRG
  && type_liq != liq_invald
  && type_liq != liq_racl
...
void DroitsRetr::Liq()
{
  if (
      (
          arr_mois(agetest) < min(arr_mois(l.AgeMinFP), arr_mois(l.AgeMinRG))
    || (duree_tot_maj == 0)
      )
    // Permet au invalide et racl d'avoir une pension
    && type_liq != liq_invald
  && type_liq != liq_racl
  ...
  if ((
      arr_mois(agetest) < arr_mois(l.AgeMinRG)
        || (
            (options->SecondLiq || options->anLeg < 2014 || int_mois(X.anaiss+agetest, X.moisnaiss + 1) < 2015 || duree_fp == 0)
        && options->tp
        && tauxliq_rg < 1
        )
  )
        // Calcul de la pension du régime privé pour ceux qui liquident au titre de l'invalidité
        && type_liq != liq_invald
        && type_liq != liq_racl
  ) {
    pension_rg     = 0;
...
void DroitsRetr::coeffTempo() {
  ...
  if(
    (!options->NoCoeffTemp)
    && ((l.an_leg >= 2015 && l.an_leg <= 2022) || (l.an_leg >= 2023 && t <= 123)) 
    && (tauxliq_rg >=1)
    && (t >= 119)
    && (X.anaiss >=1957)
    && duree_emprg > 0
