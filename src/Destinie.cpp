...
void destinieCalageSalaires(Environment envSim) {
  ...
  if(options->super_effet_hrzn_2023) { // Effet horizon associé à la réforme 2023
    for(Indiv& X : pop) {
      if(X.anaiss >= 1961) {  // pour tous les individus nés à partir de 1961
        int agefinAct_reforme_2023 = 0;
        
        // On détermine l'âge de fin d'activité
        for(int age : range(min(70,X.ageMax))) {
          if(in(X.statuts[age],Statuts_occ)) {
            agefinAct_reforme_2023 = age;
          }
        }
        
        if(agefinAct_reforme_2023 + X.anaiss >= 2023) {  // si fin d'activité après 2023
          double decalage_reforme_2023 = 2 * partTx(X.anaiss, 1961, 1968);
          int agefinActNouv_reforme_2023 = agefinAct_reforme_2023 + int(decalage_reforme_2023 + alea()); // le nouvel age de fin d'activité = age fin d'act observé + decalage compris entre 0 et 2
          
          if(agefinAct_reforme_2023+1 <= agefinActNouv_reforme_2023 && X.statuts[agefinAct_reforme_2023+1] != 0) {  // si l'age de fin d'activité à été augmenté d'au moins 1 ans, et si le statut d'activité agefinAct_reforme_2023+1 est non nul
            X.statuts[agefinAct_reforme_2023+1] = X.statuts[agefinAct_reforme_2023];  // -> on prolonge le statut d'activité T+1 par celui observé en T
            X.salaires[agefinAct_reforme_2023+1] = X.salaires[agefinAct_reforme_2023] * M->SMPT[X.date(agefinAct_reforme_2023+1)] / M->SMPT[X.date(agefinAct_reforme_2023)]; // on prolonge le salaire tout en corrigeant du SMPT
          }
          if(agefinAct_reforme_2023+2 <= agefinActNouv_reforme_2023 && X.statuts[agefinAct_reforme_2023+2] != 0) {  // idem si age décalé de 2 ans: on prolonge de 2 ans le dernier statut
            X.statuts[agefinAct_reforme_2023+2] = X.statuts[agefinAct_reforme_2023];
            X.salaires[agefinAct_reforme_2023+2] = X.salaires[agefinAct_reforme_2023] * M->SMPT[X.date(agefinAct_reforme_2023+2)] / M->SMPT[X.date(agefinAct_reforme_2023)];
          }
        }
      }
    }
...
void destinieSim(Environment envSim) {
  ...
  Union df_union = Rdin<Union>("union");
  int n_union = df_union.Id1.size();
  ...
  for(int t=50; t <= AN_FIN; t++) {
    ...
    for (Indiv& X: pop) {
      ...
      // Année de base : un seul conjoint, on attribue des droits dérivés aux veufs
      if (t == AN_BASE && X.est_present(t) && X.conjoint[age] > 0 && X.matri[age] == VEUF) {
        r_assert(X.conjoint[age] >= 0 && X.conjoint[age] <= int(pop.size()));
        Indiv& Y = pop[X.conjoint[age]];
        X.retr->SimDer(Y, t);
      }
      // Itère sur les individus décédant en t
      else if (!X.est_present(t) && X.est_present(t-1) && (X.emigrant == 0)) {
        for (auto i : range(n_union)) {
          int idConj = 0;
          if ((df_union.Id1[i] == X.Id) && (df_union.t_ageMax1[i] < df_union.t_ageMax2[i])) {
            idConj = df_union.Id2[i];
          } else if ((df_union.Id2[i] == X.Id) && (df_union.t_ageMax2[i] < df_union.t_ageMax1[i])) {
            idConj = df_union.Id1[i];
          } else {
            continue;
          }
          Indiv& Y = pop[idConj];
...
void destinieSimAgefin(Environment envSim) {
  ...
  for(auto i : range(AGE_NB)) {
    for(auto j : range(GEN_NB)) {
      for(auto k : range(3)) {
        df_retraites.push_line(k, j+GEN_MIN, i+AGE_MIN, retraites[k][j][i], actifs[k][j][i], emplois[k][j][i]);
      }
    }
  }
}
