...
/**
 * \class Retraite
 * \brief Pensions de droits directs et dérivés ainsi que le minimum vieillesse à l'âge courant
 * 
 * Pensions de droits directs et dérivés ainsi que le minimum vieillesse à l'âge courant. 
 * La classe Retraite est le coeur de la simulation.
 * 
...
public:
  ...
  double  rev_tot = 0,      ///< Réversion totale tous régimes
          ...
          rev_ag = 0,       ///< Réversion à l'AGIRC
		      rev_ag_ar=0;      ///< Réversion au régime unifié des complémentaires du privé
  ///\}
  
  double min_vieil = 0;     ///< montant perçu pour l'ASPA (anciennement : minimum vieillesse). N'est renseigné que pour la personne de référence du ménage
  
  double txCSG = 0; 		///<taux de CSG à appliquer aux retraites de base et complémentaires
  double txMal = 0;			///<taux de maladie à aplliquer aux retraites complémentaires
  
  double retraite_tot = 0;  ///< Total des pensions de droits directs et dérivés
  double retraite_net = 0;	///< Total des pensions de droits directs et dérivés nets de la CSG de la CRDS et des cotisations maladies pour les complémentaires
  ...
  */
  ...
  /**
   * \fn minvieil(int t)
   * \brief La fonction minvieil calcule le montant du minimum vieillesse auquel a droit le cas échéant le ménage.
   * \param t année de projection
   */
  void minvieil(int t);
   ...
   * 
   *  partavtliq renvoie la part de l'année avant la première liquidation et permet le calcul de la proratisation des pensions.
   *  \param t année de la simulation
   *  \return 1 si pas encore liquidé, 0 si en retraite toute l'année
   */ 
  ...
  double partavtliq(int t)
  {
    int age = X.age(t);
    if(liq)
      return liq->agefin_primoliq > 0 ? min_max(arr_mois(liq->agefin_primoliq - age, X.moisnaiss+1),  0, 1) : 1;
    else
      return 1;
  }
   ...
   * 
   *  La fonction renvoie la part de l'année avant la liquidation totale et permet le calcul de la proratisation des pensions.
   *  \param t année de la simulation
   *  \return 1 si pas encore liquidé, 0 si en retraite toute l'année
   */ 
  ...
  double partavttotliq(int t)
  {
    int age = X.age(t);
    if(totliq)
      return liq->agefin_totliq > 0 ? min_max(arr_mois(liq->agefin_totliq - age, X.moisnaiss+1),  0, 1) : 1;
    else
      return 1;
  }
   ...
   * 
   * \brief Renvoie la valeur annuelle au 31/12 des pensions de réversion touchées. 
   * \param regime régime d'affiliation
   */

  double reversion(int regime) {
    if(regime==REG_TOT) return rev_tot;
    if(regime==REG_RG) return rev_rg;
    if(regime==REG_IN) return rev_in;
    if(regime==REG_FP) return rev_fp;
    if(regime==REG_AR) return rev_ar;
    if(regime==REG_AG) return rev_ag;
	  if(regime==REG_AGIRC_ARRCO) return rev_ag_ar;
    return 0;
  }
  ...
  double pension(int regime) {
    if(regime==REG_TOT) return pension_tot;
  ...
  /**
   * \fn revenu
   * \brief Renvoie la valeur annualisée au 31/12 des revenus touchés par un individu
   **/ 
  double revenu() {
    if(liq)
      return retraite_tot;
  ...
  /**
   * \fn revenuCoupleBrut
   * \brief Renvoie la valeur annualisée au 31/12 des revenus touchés par un ménage
   **/   
  ...
  * \brief Renvoie la pension perçue à un âge donné, à un régime donné (tout régime par défaut)
  * \param age âge
  ...
  */ 
  ...
  /** 
   * \fn pension_prorat(double pension, double daterevalo, double RevaloReg, double partavtliq=0)
   * \brief Renvoie la pension percue au cours de l'année en tenant compte de la proratisation et de la revalorisation
   * \param pension montant de la pension au 31/12
   * \param daterevalo date de la relavoration des pensions
   * \param RevaloReg valeur de la revalorisation 
   * \param partavtliq  période de l'année avant la liquidation des droits retraites
   */   
  double pension_prorat(double pension, double daterevalo, double RevaloReg, double partavtliq=0);
