...
double cotisation_retraite(Indiv& X, int age, int regime) {
  ...
  if( t >=119 && options->anLeg >= 2015 && (!options->NoRegUniqAgircArrco) &&in(statut,cot_RG) && (regime==REG_AGIRC_ARRCO || regime==REG_TOT)) {
...
deque<int> regimes_cot(Indiv& X, int age) {
  ...
  if (annee < 2019 || options->anLeg < 2015 || options->NoRegUniqAgircArrco) {
...
void ecriture_indicateurs_an(int t) {
      ...
      for(auto sexe : {0,X.sexe})
      {
        ...
        if(!X.retr->liq || X.retr->liq->ageliq >= age || X.retr->liq->ageliq==0) {
          ...
          for(auto& regime : reg_cot) {
            ...
            indic.Sal_moy_31dec        .push(true, sal_tot, M->poids);
        ...
        for(auto& regime : Regimes) {
          ...
          for(auto& liqrev : X.retr->liqrevs) {
            
            int typeFPConj = pop[liqrev->idConj].typeFP;
            ...
            double pens_rev = ((regime==REG_FPE && typeFPConj != FPE) || (regime==REG_FPT && typeFPConj != FPTH)) ? 0 : rev;
            bool ind_rev = pens_rev > 0;
            ...
            indic.M_Derive            .push(ind_rev, pens_rev, M->poids/1000000.0);
            ...
            indic.Eff_Flux_Derive     .push(ind_rev && liqrev->ageliq_rev == age, 1, M->poids/1000.0);

            indic.PMoy_Flux_Derive    .push(ind_rev && liqrev->ageliq_rev == age, rev, M->poids/1000.0);
            
            indic.M_Flux_Derive       .push(ind_rev && liqrev->ageliq_rev == age, pens_rev, M->poids/1000000.0); 
            indic.M_Pensions_ma       .push(ind_rev, pens_rev, M->poids/1000000.0); // valeur au 31/12
            indic.M_Pensions          .push(ind_rev, pens_rev, M->poids/1000000.0);
            indic.M_MV                .push(ind_mv, pens_rev, M->poids/1000000.0);
            indic.M_Flux_MV           .push((X.dateMinFlux == t) && ind_mv, pens_rev, M->poids/1000000.0);
            ...
            // pension DD dans le régime
            double pens_dd = X.retr->pension(reg);
            double retraite_tot = pens_rev + pens_dd;
            
            indic.Eff_DD_et_Derive                    .push(ind_rev && pens_dd > 0, 1, M->poids/1000.0);
            indic.M_Derive_parmi_indiv_DD             .push(ind_rev && pens_dd > 0, pens_rev, M->poids/1000000.0);
            indic.M_DD_parmi_indiv_Derive             .push(ind_rev, pens_dd, M->poids/1000000.0);
            indic.M_Pensions_parmi_indiv_DD           .push(ind_rev && pens_dd > 0, pens_rev, M->poids/1000000.0);
            indic.M_Pensions_parmi_indiv_Derive       .push(ind_rev, retraite_tot, M->poids/1000000.0);
            indic.M_Pensions_parmi_indiv_DD_et_Derive .push(ind_rev && pens_dd > 0, retraite_tot, M->poids/1000000.0);
            
            // Eviter le double comptage parmi les réversions
            if (ind_rev) break;
          }  
        ...
        for(auto& regime : Regimes) {
          ...
          // pension droits dérivés dans le régime
          double rev = X.retr->reversion(reg);
          bool ind_rev = rev > 0;
          
          //retraite totale
          double retraite_tot = pens_dd + rev;
          ...
          if(pens_dd > 0) {
            ...
            if (reg==REG_TOT) {
              pension_min_ma = (X.retr->liq->indic_mg>0)*X.retr->pension_prorat(age,REG_FP) + 
                               (X.retr->liq->indic_mc>0)*X.retr->pension_prorat(age,REG_RG) +
                               (X.retr->liq->indic_mc_in>0)*X.retr->pension_prorat(age,REG_IN);
              }
            else {
              pension_min_ma = X.retr->pension_prorat(age,reg);
              }
            
            double pension_min=0.0;
            if (reg==REG_TOT) {
              pension_min = (X.retr->liq->indic_mg>0)*X.retr->pension(REG_FP) + 
                            (X.retr->liq->indic_mc>0)*X.retr->pension(REG_RG) + 
                            (X.retr->liq->indic_mc_in>0)*X.retr->pension(REG_IN);
              }
            ...
            indic.Eff_DD_seul               .push(!ind_rev, 1, M->poids/1000.0);
            indic.M_DD                      .push(true, pens_dd, M->poids/1000000.0);
            indic.M_Pensions_parmi_indiv_DD .push(true, pens_dd, M->poids/1000000.0);
            ...
            indic.PMoy_DD                   .push(true, pens_dd, M->poids) ;
            indic.PMoy_tot_parmi_indiv_DD   .push(true, retraite_tot, M->poids) ;
          }
          ...
          if(ind_liq_dd && (X.retr->liq->pensionliq(reg) + X.retr->liq->VFU(reg)) > 0) {
            ...
            if (reg==REG_TOT) {
              ...
              partavt = X.retr->partavtliq(t);
              }
            else {
              partavt = X.retr->partavttotliq(t);
              }
            ...
            if (reg==REG_TOT) {
              ...
              (age==X.retr->liq->ageliq && age>X.retr->liq->ageprimoliq) ? (dr.VFU(REG_RG)==0)*dr.pension_prorat(age,REG_RG) +
                                                                           (dr.VFU(REG_AR)==0)*dr.pension_prorat(age,REG_AR) +
                                                                           (dr.VFU(REG_AG)==0)*dr.pension_prorat(age,REG_AG) + 
                                                                           dr.pension_prorat(age,REG_IN) : 
              (age==X.retr->liq->ageliq && age==X.retr->liq->ageprimoliq) ? dr.pension_prorat(age,REG_FP) +
                                                                            (dr.VFU(REG_RG)==0)*dr.pension_prorat(age,REG_RG) +
                                                                            (dr.VFU(REG_AR)==0)*dr.pension_prorat(age,REG_AR) +
                                                                            (dr.VFU(REG_AG)==0)*dr.pension_prorat(age,REG_AG) +
                                                                            dr.pension_prorat(age,REG_IN) : 0;
            }
            else {
              pensionliq_ma = dr.pension_prorat(age,reg);
              }
            ...
            if (reg==REG_TOT) {
              ...
              (age==X.retr->liq->ageliq && age>X.retr->liq->ageprimoliq) ? (dr.VFU(REG_RG)==0)*dr.pensionliq(REG_RG) +
                                                                           (dr.VFU(REG_AR)==0)*dr.pensionliq(REG_AR) + 
                                                                           (dr.VFU(REG_AG)==0)*dr.pensionliq(REG_AG) +
                                                                           dr.pensionliq(REG_IN) : 
              (age==X.retr->liq->ageliq && age==X.retr->liq->ageprimoliq) ? dr.pensionliq(REG_FP) +
                                                                            (dr.VFU(REG_RG)==0)*dr.pensionliq(REG_RG) +
                                                                            (dr.VFU(REG_AR)==0)*dr.pensionliq(REG_AR) +
                                                                            (dr.VFU(REG_AG)==0)*dr.pensionliq(REG_AG) +
                                                                            dr.pensionliq(REG_IN) :0;
              }
            else {
              pensionliq = dr.pensionliq(reg);
              }
            ...
            if (reg==REG_TOT) {
              ...
              (age==X.retr->liq->ageliq && age>X.retr->liq->ageprimoliq) ? (dr.indic_mc>0)*(dr.VFU(REG_RG)==0)*dr.pensionliq(REG_RG) + 
                                                                           (dr.indic_mc_in>0)*dr.pensionliq(REG_IN) : 
              (age==X.retr->liq->ageliq && age==X.retr->liq->ageprimoliq) ? (dr.indic_mg>0)*dr.pensionliq(REG_FP) + 
                                                                            (dr.indic_mc>0)*(dr.VFU(REG_RG)==0)*dr.pensionliq(REG_RG) + 
                                                                            (dr.indic_mc_in>0)*dr.pensionliq(REG_IN): 0;
              }
            else {
              pensionliq_min = dr.pensionliq(reg);
              }
            ...
            if (reg==REG_TOT) {
              ...
              (age==X.retr->liq->ageliq && age==X.retr->liq->ageprimoliq) ? (dr.indic_mg>0)*dr.pension_prorat(age,REG_FP) + 
                                                                            (dr.indic_mc>0)*(dr.VFU(REG_RG)==0)*dr.pension_prorat(age,REG_RG) +
                                                                            (dr.indic_mc_in>0)*dr.pension_prorat(age,REG_IN) : 0;
              }
            else {
              pensionliq_min_ma = dr.pension_prorat(age,reg);
              }
            ...
            if (reg==REG_TOT) {
              ...
              majo_min_ma = dr.majo_min(reg)*(1-partavt);
              }
          ...
          indic.Pmoy_tot                  .push(retraite_tot > 0, retraite_tot, M->poids);
          ...
          /* indic.Eff_DD_MV         .push(ind_dd && ind_mv, 1, M->poids/1000.0);
           ...
           //indic.Eff_Flux_DRseul_MV     .push( ind_rev && ind_mv && liqrev->ageliq_rev == age, 1, M->poids/1000.0); //X.retr->liq->age_liq(reg)!=age&& X.retr->liqrev->ageliq_rev == age
        ...
        for(auto& regime : Regimes) {
          ...
          double min_vieil = X.retr->min_vieil;
          bool ind_mv = min_vieil > 0;
          // On doit vérifier plus manuellement si l'individu possède une pension dans la FPE et la FPTH
          int reg = (regime == REG_FPE || regime == REG_FPT) ? REG_FP : regime;
          int type_fp_reg = regime == REG_FPE ? FPE : 
            regime == REG_FPT ? FPTH : 9999;
          double pens_dd = (regime == REG_FPE && X.typeFP != FPE) || (regime == REG_FPT && X.typeFP != FPTH) ? 0 :
            X.retr->pension(reg);
          
          // Cherche l'existence d'une pension de réversion dans le régime
          bool ind_pens_rev_reg = false;
          for(auto& liqrev : X.retr->liqrevs) {
            int typeFPConj = pop[liqrev->idConj].typeFP;
            double pens_rev = ((regime == REG_FPE && typeFPConj != FPE) || (regime == REG_FPT && typeFPConj != FPTH)) ? 0 :
              X.retr->reversion(reg);
            
            ind_pens_rev_reg = (!in(regime,{REG_FPE, REG_FPT}) && pens_rev > 0) ||
              (typeFPConj == type_fp_reg && pens_rev > 0);
            
            if(ind_pens_rev_reg) break;
          }
          bool ind_pens_dd_reg = (!in(regime,{REG_FPE, REG_FPT}) && pens_dd > 0) ||
            (X.typeFP == type_fp_reg && pens_dd > 0);
          bool ind_pens_reg = ind_pens_dd_reg || ind_pens_rev_reg;
          
          indic.Eff_MV        .push(ind_mv && ind_pens_reg, 1, M->poids/1000.0);
          indic.Eff_Flux_MV   .push((X.dateMinFlux == t) && ind_mv && ind_pens_reg, 1, M->poids/1000.0);
          indic.M_MV          .push(ind_mv, pens_dd, M->poids/1000000.0);
          indic.M_Flux_MV     .push((X.dateMinFlux == t) && ind_mv, pens_dd, M->poids/1000000.0);
        }
      }
  ...
  static Rdout indicateurs_an("Indicateurs_an", {
    ...
    "Eff_Min_Flux", "M_Min_Flux", "Eff_Min", "M_Min", "Eff_Derive", "M_Derive", "M_Derive_parmi_indiv_DD", "M_DD_parmi_indiv_Derive",
    "Eff_Derive_seul", "Eff_DD_seul", "Eff_DD_et_Derive",
    "M_Pensions__Derive_seul", "Eff_Flux_Derive", 
    "M_Pensions", "M_Pensions_parmi_indiv_DD", "M_Pensions_parmi_indiv_Derive", "M_Pensions_parmi_indiv_DD_et_Derive",
    ...
    "Eff_DD_Deces", "Eff_Derive_Deces", "M_DD_Flux", "M_Flux_Derive", "M_DD_Deces", "M_Derive_Deces",
    "PMoy_DD", "PMoy_tot_parmi_indiv_DD", "Pmoy_tot",
    ...
    "M_MICO_flux_ma", "M_MICO_stock_ma", "Sal_Plaf_31dec", "Sal_Tot_31dec", "Sal_moy_31dec", "TauxRemplace"},
    "regime", regimes_labels, "sexe", sexe_labels);
  ...
  for(int s =0; s < 3; s++) 
    for(int r =0; r < 10; r++)
    {
      ...
      indicateurs_an.push_line(
        ...
        i.Eff_Min_Flux, i.M_Min_Flux, i.Eff_Min, i.M_Min, i.Eff_Derive, i.M_Derive, i.M_Derive_parmi_indiv_DD, i.M_DD_parmi_indiv_Derive,
        i.Eff_Derive_seul, i.Eff_DD_seul, i.Eff_DD_et_Derive,
        i.M_Derive_seul, i.Eff_Flux_Derive, 
        i.M_Pensions, i.M_Pensions_parmi_indiv_DD, i.M_Pensions_parmi_indiv_Derive, i.M_Pensions_parmi_indiv_DD_et_Derive,
        ...
        i.Eff_Derive_Deces, i.M_DD_Flux, i.M_Flux_Derive, i.M_DD_Deces, i.M_Derive_Deces,
        i.PMoy_DD, i.PMoy_tot_parmi_indiv_DD, i.Pmoy_tot,
        ...
        i.M_Min_Flux_ma,i.M_Min_ma, i.M_Pensions_ma, i.M_MICO_flux_ma, i.M_MICO_stock_ma, i.Sal_Plaf_31dec, i.Sal_Tot_31dec, i.Sal_moy_31dec, i.TauxRemplace
      );
    }
}
