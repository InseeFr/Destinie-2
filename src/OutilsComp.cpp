...
bool test_liq_racl(Indiv& X, DroitsRetr& dr, Leg& l, double agetest) {
  // test de l'éligibilité au dispositif de retraite anticipée 
  // pour carrières longues défini par la réforme de 2003
  if (l.LegDRA < 2003 || dr.t < 104 || options->NoRetrAntCarrLongues) {
    return false;
  }
  
  // détermination de l'âge de début d'activité
  // s'arrête au premier statut occupé ou pour un age >= 30
  int a = 14;
  while (!in(X.statuts[a], Statuts_occ) && a < 30) {
    a++;
  }
  double agedeb = a;
  ...
  int j = 0;
  // Présuppose d'avoir ordonné les âges de RACL dans l'ordre croissant
  while (j < 5 && l.AgeDRA[j] && l.AgeDRA[j] < 99) {
    if (
        (agedeb + 1 <= l.DebActCibDRA[j])  
      && (dr.dureecotdra_tot >= l.DureeCotCibDRA[j])
      && (dr.duree_tot_maj >= l.DureeValCibDRA[j])
      && (agetest >= l.AgeDRA[j])
...
bool TestLiq(Indiv & X, DroitsRetr & dr, Leg &l, double agetest)
{
  ...
  dr.durees_et_decote_surcote();
  
  // Retour rapide si l'individu ne peut pas liquider
  if(dr.duree_tot == 0 || arr_mois(agetest) < arr_mois(dr.AgeMinGlobal())) {
  ...
  if(in(X.statuts[agearr], statuts_invalid) && agetest >= l.AgeMinInv && agetest >= 50)  {//Ajout borne minimale a 50 ans par precaution 
    // TODO vérifier effets de bord sur l.AgeMinRG X.AgeMinRG ?
    // Spécification du type de liquidation en amont de Liq() pour qu'il puisse utiliser cette information
  ...
  // Liquidation si RACL
  if(test_liq_racl(X, dr, l, agetest)) {
    // Risques possible d'empêcher une liquidation inaptitude en définissant type_liq avant d'appliquer Liq()
    // Mais l'information type_liq est nécessaire pour correctement calculer les paramètres de liquidation
    if(!dr.type_liq) dr.type_liq = liq_racl;
    dr.Liq();
    return true;
  }
  
  // Liquidation impossible si age inférieur à l'âge d'ouverture des droits
  if (arr_mois(agetest) < arr_mois(dr.AgeMin())) {
    return false;
  }
